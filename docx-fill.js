...
const personText=(p,isBuyer=false)=>`${p.nom.toUpperCase()} ${p.prenoms}, né(e) le ${p.naissance||"___"} à ${p.lieuNaissance||"___"}, ${p.profession||"___"}, demeurant ${p.adresse||"___"}, situation : ${p.situation}${["Marié(e)","Pacsé(e)"].includes(p.situation)?` le ${p.unionDate||"___"} à ${p.unionLieu||"___"}`:""}${isBuyer&&p.primo?", primo-accédant":""}.`;
