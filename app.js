...
</div><div class="importRow">${p.type==="morale"?importButton(kind==="vendeurs"?"vendeur":"acquereur","le KBIS","kbis",i):importButton(kind==="vendeurs"?"vendeur":"acquereur","la CNI / le passeport","cni",i)}</div></div>`).join("")+`<button data-add="${kind}">Ajouter un ${kind==="vendeurs"?"vendeur":"acquéreur"}</button>`}
